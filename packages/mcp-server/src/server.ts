...
export function buildServer() {
  ...
  server.registerTool("magen3_get_compliance_controls_status", { title: "Get Compliance Provider Status", description: "Return sanitized Compliance provider capabilities, provider health, feed state, and request-independent configuration status. No credentials, raw personal data, or unbounded provider payloads are exposed.", inputSchema: z.object({}), annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true } }, async () => handlers.getComplianceControlsStatus());
  server.registerTool("magen3_get_continuous_risk_monitoring_status", { title: "Get Continuous Risk Monitoring Status", description: "Return deterministic monitoring capability and scheduler status without exposing operational evidence or credentials.", inputSchema: z.object({}), annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true } }, async () => handlers.getContinuousRiskMonitoringStatus());
  server.registerTool("magen3_get_monitoring_alerts", { title: "Get Agent Monitoring Alerts", description: "Return bounded monitors and alert/recovery state for the authenticated connected agent.", inputSchema: z.object({}), annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true } }, async () => handlers.getMonitoringStatus());

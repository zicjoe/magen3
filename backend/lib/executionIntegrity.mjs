...
function canonicalIntentPayload(request = {}) {
  return stableValue({
    ...
    tokenPermission: request.tokenPermission ? stableValue({
      kind: clean(request.tokenPermission.kind),
      standard: clean(request.tokenPermission.standard).toUpperCase(),
      network: clean(request.tokenPermission.network).toLowerCase(),
      chainId: clean(request.tokenPermission.chainId),
      tokenContract: clean(request.tokenPermission.tokenContract).toLowerCase(),
      tokenIdentifierType: clean(request.tokenPermission.tokenIdentifierType),
      owner: clean(request.tokenPermission.owner).toLowerCase(),
      spender: clean(request.tokenPermission.spender).toLowerCase(),
      intendedSpender: clean(request.tokenPermission.intendedSpender).toLowerCase(),
      approvalAmount: request.tokenPermission.approvalAmount ?? null,
      approvalAmountAtomic: clean(request.tokenPermission.approvalAmountAtomic),
      intendedTransactionAmount: request.tokenPermission.intendedTransactionAmount ?? null,
      unlimited: request.tokenPermission.unlimited === true,
      deadline: clean(request.tokenPermission.deadline),
      nonce: clean(request.tokenPermission.nonce),
      permitIdentifier: clean(request.tokenPermission.permitIdentifier),
      permitSignatureHash: clean(request.tokenPermission.permitSignatureHash).toLowerCase(),
      reusable: request.tokenPermission.reusable === true,
      oneTime: request.tokenPermission.oneTime === true,
      resetAfterUse: request.tokenPermission.resetAfterUse === true,
      operatorApprovalForAll: request.tokenPermission.operatorApprovalForAll === true,
      batch: request.tokenPermission.batch ?? [],
    }) : null,
    x402: {

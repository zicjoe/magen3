...
import { evaluateBridgeProviderIntegration } from "./bridgeProviderIntegration.mjs";
...
function withStructuredResult({
  ...
  bridgeProviderIntegrationContext = null,
  ...
  valueExposureContext = null,
  walletBehavioralControlsContext = null,
  assetIdentityContext = null,
  assetContractRiskContext = null,
  ...
  return {
    ...
    valueExposureContext,
    walletBehavioralControlsContext,
    assetIdentityContext,
    assetContractRiskContext,
...
export function evaluateAction({ request, agents, policies, auditLogs, emergencyPauses = [], threatIntelligence = {}, oracleValidation = {}, marketRiskSignals = {}, complianceControls = {} }) {
  ...
  const assetIdentityResult = evaluateAssetIdentity({ request, policy });
  request.assetIdentity = assetIdentityResult.context;
  ...
  const walletBehavioralControlsResult = evaluateWalletBehavioralControls({ request, policy, auditLogs });
  ...
  checksPassed.push(...valueExposureLimitsResult.checksPassed);
  checksFailed.push(...valueExposureLimitsResult.checksFailed);
  moduleFindings.push(...valueExposureLimitsResult.findings);
  score += valueExposureLimitsResult.scoreDelta;

  checksPassed.push(...walletBehavioralControlsResult.checksPassed);
  checksFailed.push(...walletBehavioralControlsResult.checksFailed);
  moduleFindings.push(...walletBehavioralControlsResult.findings);
  score += walletBehavioralControlsResult.scoreDelta;
  ...
  const statefulSimulation = evaluateStatefulSimulationEvidence({ request, policy });
  ...
  checksPassed.push(...statefulSimulation.checksPassed);
  checksFailed.push(...statefulSimulation.checksFailed);
  ...
  const tradingRouteIntegrityResult = evaluateTradingRouteIntegrity({ request, policy });
  checksPassed.push(...tradingRouteIntegrityResult.checksPassed);
  checksFailed.push(...tradingRouteIntegrityResult.checksFailed);
  ...
  const bridgeProviderIntegrationResult = evaluateBridgeProviderIntegration({ request, policy });
  checksPassed.push(...bridgeProviderIntegrationResult.checksPassed);
  checksFailed.push(...bridgeProviderIntegrationResult.checksFailed);
  moduleFindings.push(...bridgeProviderIntegrationResult.findings);
  score += bridgeProviderIntegrationResult.scoreDelta;
  ...
  return withStructuredResult({
    ...
    bridgeProviderIntegrationContext: bridgeProviderIntegrationResult.context,

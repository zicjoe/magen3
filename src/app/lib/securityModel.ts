...
export const PROTECTION_MODULE_CATALOG: ProtectionArea[] = [
  ...
  protectionArea({
    ...
    description: "Applies deterministic policy rules and routes questioned actions into autonomous remediation, independent verification, or explicit human authorization.",
    capabilities: ALL_CAPABILITIES,
    controls: [
      ...
      { id: "review-thresholds", name: "Review thresholds", description: "Pauses high-value or policy-sensitive requests for the configured autonomous, balanced, or human-governed resolution strategy.", status: "Live", configurable: true },
  ...
  protectionArea({
    ...
    controls: [
      ...
      { id: "bridge-provider-integration", name: "Real bridge provider integration", description: "Fetches an attested Across testnet quote, constructs the exact unsigned source transaction, binds it to the protected intent, and tracks destination delivery through reconciliation. Signing and submission remain outside Magen3.", status: "Foundation Available", configurable: true },
...
export function calculateSecurityCoverage(
  ...
  const reviewResolutionMode = String(policy?.structuredRules?.reviewResolutionMode || "Autonomous");
  const humanApprovalRelevant = reviewRelevant && reviewResolutionMode !== "Autonomous";
  ...
  const approvalOrganizationalRelevant = humanApprovalRelevant && capabilities.some((item) => ["Treasury Operations", "Enterprise Automation"].includes(item));
  ...
  const checks: CoverageCheck[] = [
    ...
    { id: "review-threshold", label: "Review threshold", weight: 8, passed: !reviewRelevant || Number(policy?.approvalThreshold) > 0, detail: !reviewRelevant ? "Not required by the selected capabilities." : Number(policy?.approvalThreshold) > 0 ? `Review required above ${policy?.approvalThreshold} CSPR using the ${reviewResolutionMode} strategy.` : "No review threshold is configured.", recommendation: "Configure a review threshold for higher-value actions.", page: "policies" },
    { id: "approval-workflow", label: "Human approval workflow", weight: 6, passed: !humanApprovalRelevant || (approvalConfigured && approvalOperational && (!approvalSignatureRequired || approvalSignatureOperational)), detail: !humanApprovalRelevant ? `Not required by the ${reviewResolutionMode} review strategy for ordinary reviews.` : approvalConfigured ? approvalOperational ? approvalSignatureRequired ? approvalSignatureOperational ? `Human-escalated reviews are bound to ${approvalRequiredCount} verified Casper Wallet signature${approvalRequiredCount === 1 ? "" : "s"}, and signed quorum evidence has been observed.` : `The workflow requires ${approvalRequiredCount} cryptographically verified approval${approvalRequiredCount === 1 ? "" : "s"}, but no verified reviewer signature has been observed yet.` : `Human-escalated reviews are bound to ${approvalRequiredCount} approval${approvalRequiredCount === 1 ? "" : "s"} and the workflow has been observed.` : `The workflow is configured for ${approvalRequiredCount} approval${approvalRequiredCount === 1 ? "" : "s"}, but no human-escalated review has been observed yet.` : "Human-escalated reviews do not yet have a complete approver and quorum configuration.", recommendation: approvalConfigured ? "Trigger a human-escalated Review Required request and resolve it in the Approval Queue." : "Enable Human Approval & Quorum only when the review strategy can escalate to people.", page: "policies" },
    { id: "organizational-approval", label: "Organizational approval rules", weight: 5, passed: !approvalOrganizationalRelevant || (approvalOrganizationalConfigured && approvalOrganizationalOperational), detail: !approvalOrganizationalRelevant ? "Not required by the selected capabilities." : approvalOrganizationalConfigured ? approvalOrganizationalOperational ? `${approvalGroups.length} named approver group${approvalGroups.length === 1 ? " is" : "s are"} configured and a resolved tier has satisfied its role quorum.` : `${approvalGroups.length} named approver group${approvalGroups.length === 1 ? " is" : "s are"} configured, but no successful tier and group-quorum evaluation is visible yet.` : "Treasury or enterprise automation does not yet have complete named groups and a tier or organization-default quorum rule.", recommendation: approvalOrganizationalConfigured ? "Submit a Review Required treasury or enterprise example and complete every required role in the Approval Queue." : "Enable Approval Escalation & Organizational Quorum, define named groups, and add at least one deterministic tier or organization-default role requirement.", page: approvalOrganizationalConfigured ? "human-approval" : "policies" },
...
export function deriveIntegrationHealth(
  ...
  const policyApprovalFindings = latest?.moduleFindings?.filter((finding) => finding.module === "Policy & Approval Controls") || [];
  const autonomousReviewFindings = policyApprovalFindings.filter((finding) => finding.rule === "Autonomous review resolution");
  const autonomousReviewFailed = autonomousReviewFindings.some((finding) => finding.status === "fail");
  const autonomousReviewUnavailable = autonomousReviewFindings.some((finding) => finding.status === "unavailable");
  const autonomousReviewPending = autonomousReviewFindings.some((finding) => finding.status === "warning");
  const autonomousReviewHealth = autonomousReviewFindings.length === 0 ? "unknown" : autonomousReviewFailed ? "attention" : autonomousReviewUnavailable ? "unavailable" : autonomousReviewPending ? "pending" : "observed";
  const humanApprovalRules = new Set(["Human approval quorum", "Human approval workflow", "Cryptographic reviewer signature", "Organizational approval quorum", "Organizational approval rules"]);
  const approvalFindings = policyApprovalFindings.filter((finding) => humanApprovalRules.has(finding.rule));
  ...
  const bridgeProviderFindings = latest?.moduleFindings?.filter((finding) => finding.module === "Bridge Provider Integration") || [];
  const bridgeProviderFailed = bridgeProviderFindings.some((finding) => finding.status === "fail");
  const bridgeProviderUnavailable = bridgeProviderFindings.some((finding) => finding.status === "unavailable");
  const bridgeProviderWarned = bridgeProviderFindings.some((finding) => finding.status === "warning");
  const bridgeProviderPassed = bridgeProviderFindings.some((finding) => finding.rule === "Bridge payload binding" && finding.status === "pass");
  const bridgeProviderHealth = bridgeProviderFindings.length === 0 ? "unknown" : bridgeProviderFailed ? "attention" : bridgeProviderUnavailable ? "unavailable" : bridgeProviderWarned ? "pending" : bridgeProviderPassed ? "healthy" : "observed";
  ...
  const checks = [
    ...
    { label: "Autonomous review resolution", status: autonomousReviewHealth, detail: autonomousReviewFindings.length === 0 ? "No autonomous-remediation finding is available for the latest request." : autonomousReviewFailed ? "The latest autonomous remediation failed and requires a new policy decision." : autonomousReviewUnavailable ? "The remediation path is unavailable and cannot authorize execution." : autonomousReviewPending ? "The latest request is paused while the external agent supplies the required evidence or corrected parameters." : "The autonomous review-resolution path was evaluated." },
    { label: "Human approval workflow", status: approvalHealth, detail: approvalFindings.length === 0 ? "No human-escalated approval finding is available for the latest request." : approvalFailed ? "The latest human-escalated request was rejected or expired." : approvalUnavailable ? "Human approval was explicitly required without a usable approver configuration." : approvalWarned ? "The latest human-escalated request is waiting for its configured approval quorum." : "The human approval workflow was evaluated." },
    ...
    { label: "Bridge Provider Integration", status: bridgeProviderHealth, detail: bridgeProviderFindings.length === 0 ? "No real bridge-provider finding is available for the latest request." : bridgeProviderFailed ? "The provider quote, attestation, testnet boundary, or exact source-payload binding failed." : bridgeProviderWarned ? "The provider evidence requires review before signing." : bridgeProviderUnavailable ? "The configured testnet provider or evidence attestation was unavailable and did not count as a pass." : bridgeProviderPassed ? "An attested Across testnet quote and exact unsigned source transaction matched the protected intent." : "Bridge Provider Integration was evaluated without a complete pass." },
...
export const realBridgeProviderIntegrationMilestone = { name: "Real Bridge Provider Integration", status: "Foundation Available", description: "Attested Across testnet quotes, exact unsigned source transaction construction, payload binding, and destination-delivery reconciliation; no mainnet, signing, or submission." } as const;
export const continuousRiskMonitoringMilestone = { name: "Continuous Risk Monitoring", status: "Live", description: "Deterministic scheduled and manual evaluations over existing Magen3 state with persistent checkpoints, deduplicated alerts, acknowledgement, recovery detection, and bounded provider/reconciliation monitoring." } as const;
